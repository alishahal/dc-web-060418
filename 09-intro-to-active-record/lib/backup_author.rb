...
class BackupAuthor
    ...
    def self.make_object_from_row(row)
        # [1, "Mark Twain"]
        BackupAuthor.new(row[1], row[0])
    end
    
    def self.create(name)
        author = BackupAuthor.new(name)
        author.save
    end
    ...
    def self.find_by_name(name)
        ...
        if results
            BackupAuthor.make_object_from_row(results)
        else
            return nil
        end
    end
    ...
    def self.find_or_create(name)
        author = BackupAuthor.find_by_name(name)
        author ||= BackupAuthor.create(name)
    end
    ...
    def self.find(id)
        ...
        BackupAuthor.make_object_from_row(results[0])
    end
    ...
    def self.all
        ...
        results.map do |row|
            BackupAuthor.make_object_from_row(row)
        end
    end
